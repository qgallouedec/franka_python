...
def publisher(name: str, data_class: Dataclass, timeout: Optional[float] = None) -> rospy.Publisher:
    ...
    while not has_subscriber(pub):  # while no subscriber
        if timeout is not None and rospy.Time.now() - start > rospy.Duration(secs=timeout):
            ...
            try:
                ...
                rospy.logdebug("Waiting for subscriber for topic %s interrupted" % name)
                raise e
...
def subscriber(
    name: str, data_class: Dataclass, callback: Callable[[Message], None], timeout: Optional[float] = None, tcp_nodelay=False
) -> rospy.Subscriber:
    ...
    if timeout != -1:  # wait for a message to be received if timeout != 1
        try:
            ...
            rospy.logdebug("Waiting for topic %s interrupted" % name)
            raise e
    ...
    return rospy.Subscriber(name, data_class, callback, tcp_nodelay=tcp_nodelay)
